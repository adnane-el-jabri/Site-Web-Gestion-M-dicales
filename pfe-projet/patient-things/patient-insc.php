...
if (isset($_POST['envoi'])) {
    if (!empty($_POST['fn']) and !empty($_POST['ln']) and !empty($_POST['mb']) and !empty($_POST['cin']) and !empty($_POST['vle']) and !empty($_POST['rg']) and !empty($_POST['ad']) and !empty($_POST['mdp'])) {
        ...
        $mdp = sha1($_POST['mdp']);
        $insertuser = $bdd->prepare('INSERT INTO patientmed(fn, ln, mb, cin, vle, rg, ad, mdp)VALUES(?, ?, ?, ?, ?, ?, ?, ?)');
        $insertuser->execute(array($fn, $ln, $mb, $cin, $vle, $rg, $ad, $mdp));
        $recupuser = $bdd->prepare('SELECT * FROM patientmed WHERE fn = ? and ln = ? and mb = ? and cin = ? and vle = ? and rg = ? and ad = ? and mdp = ?');
        ...
        if ($recupuser->rowCount() > 0) {
            ...
            $_SESSION['ad'] = $ad;
            $_SESSION['mdp'] = $mdp;
            $_SESSION['id_patient'] = $recupuser->fetch()['id_patient'] ;
